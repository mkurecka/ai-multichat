...
import { getModels, getAuthToken, logoutUser, handleGoogleCallback, getChatHistory } from './services/api';
...
function App() {
    ...
    const [loading, setLoading] = useState(false); // Restore global loading potentially
    ...
    const GoogleCallbackHandler = () => {
        ...
        return (_jsxs("div", { children: ["Processing login...", authLoading && _jsx("p", { children: "Loading..." }), error && _jsxs("p", { style: { color: 'red' }, children: ["Error: ", error] })] }));
    };
    ...
    return (_jsxs(Routes, { children: [_jsx(Route, { path: "/login", element: _jsx(LoginPage, { googleClientId: GOOGLE_CLIENT_ID, redirectUri: calculatedRedirectUri }) }), _jsx(Route, { path: OAUTH_CALLBACK_PATH, element: _jsx(GoogleCallbackHandler, {}) }), _jsxs(Route, { path: "/", element: _jsx(ProtectedRoute, { children: _jsx(Layout, { userEmail: userInfo?.email || null, onLogout: handleLogout, chatHistory: chatHistory, activeThreadId: activeThreadId, onNewChat: handleNewChat, onSelectThread: handleSelectThread }) }), children: [_jsx(Route, { index: true, element: _jsx(ChatPage, { models: models, activeThreadId: activeThreadId }) }), _jsx(Route, { path: "templates", element: _jsx(PromptTemplatePage, { models: models }) })] }), _jsx(Route, { path: "*", element: _jsx(Navigate, { to: "/", replace: true }) })] }));
}
export default App;
